...
from main_win.home import Ui_MainWindow
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # self.playButton.clicked.connect(self.start_reading)
        self.runButton.clicked.connect(self.run_or_continue)
        self.stopButton.clicked.connect(self.stop)
        self.resultWidget.itemClicked.connect(self.play_sound)
    ...
    def close_result(self):
        if self.resultWidget.isVisible():
            self.unfold_result(400, 70, False)
            # self.resultWidget.setVisible(False)
        else:
            self.unfold_result(70, 400)
            self.resultWidget.setVisible(True)

    def unfold_result(self, start_value, end_value, hide_=True):
        self.animation = QPropertyAnimation(self.groupBox, b"maximumWidth")
        if not hide_:
            self.animation.finished.connect(lambda: self.resultWidget.setVisible(False))
    ...
    def chose_cam(self):
        try:
            ...
            x = self.comboBox.mapToGlobal(self.cameraButton.pos()).x()
            y = self.comboBox.mapToGlobal(self.cameraButton.pos()).y()
    ...
    def run_or_continue(self):
        ...
        if self.runButton.isChecked():
            ...
            self.statistic_msg('File:{} is detecting under model:{}'.
    ...
    def play_sound(self):
        index = self.resultWidget.currentRow()
        print(index)
        name = self.resultname_catcher[index]
        result = self.results[index]
        self.start_reading(result, name)

    # def start_reading(self, result, name):
    #     if not os.path.exists('ttsmp3'):
    #         os.makedirs('ttsmp3')

    #     file_name = f'ttsmp3/{name}.mp3'  # generate filename
    #     print(file_name)
    #     try:
    #         engine = pyttsx3.init()
    ...
    def show_statistic(self, statistic_dic):
        try:
            ...
            self.resultname_catcher = [str(i[0]) for i in statistic_dic]
            # print(statistic_dic)
            data = {}
            for i in range(len(self.resultname_catcher)):
                # print (i)
                resultname = str(self.resultname_catcher[i])
                # print(resultname)
                path = "dataInfo"
                files = os.listdir(path)
                # print(files)

                position = path + '\\' + resultname
                sublist = os.listdir(position)
                for file in sublist:
                    random_file = random.choice(sublist)
                    infotxt = position + '\\' + random_file
                    # print (random_file)
                # print (infotxt)
                with open(infotxt, "r", encoding='utf-8') as f:
                    data[i] = (f.readlines())
                    # print (data[i])

            value = list(data.values())
            self.value_catcher = [str(d[0]) for d in value]
    ...
    #         file_name = f'ttsmp3/{self.resultname_catcher[i]}.mp3' # generate filename
    #         print(file_name)
    ...
    #             self.statistic_msg('Playing Sounds')
    #             time.sleep(1)
    #             # self.start_reading(self.value_catcher)
    #         self.resultWidget.addItems(self.results)
    #     except Exception as e:
    #         print(repr(e))
    ...
    def closeEvent(self, event):
        ...
        with open(config_file, 'w', encoding='utf-8') as f:
            f.write(config_json)
        MessageBox(self.closeButton, title='Tips', text='Closing the program', time=2000, auto=True).exec_()
        sys.exit(0)
